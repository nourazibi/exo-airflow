...
def pipeline_dvf():
    ...
    def verifier_sources() -> dict:
        ...
        try:
            ...
            logger.exception("API DVF inaccessible depuis le conteneur Airflow")
        ...
        if not statuts["dvf_api"]:
            logger.warning(
                "L'API DVF semble inaccessible à cette étape. "
                "Le téléchargement réel sera retenté dans telecharger_dvf."
            )
    ...
    def telecharger_dvf(statuts: dict) -> str:
        ...
        local_path = os.path.join(tempfile.gettempdir(), f"dvf_{annee}.zip")

        logger.info("Téléchargement du fichier DVF vers : %s", local_path)
                    ...
                    for chunk in resp.iter_content(chunk_size=8192):
                        if chunk:
                            ...
                            if total_bytes >= prochain_log:
                                logger.info(
                                    "Progression téléchargement : %.2f Mo",
        ...
        if not os.path.exists(local_path):
            raise AirflowException("Le fichier DVF n'a pas été créé.")

        taille = os.path.getsize(local_path)
        if taille < 1000:
            raise AirflowException(f"Le fichier téléchargé est trop petit ({taille} octets).")

        logger.info("Téléchargement terminé. Taille = %.2f Mo", taille / (1024 * 1024))
        return local_path

    @task(task_id="stocker_hdfs_raw")
    def stocker_hdfs_raw(local_path: str) -> str:
        annee = datetime.now().year
        hdfs_filename = f"dvf_{annee}.zip"
    ...
    def traiter_donnees(hdfs_path: str) -> dict:
        ...
        if not zip_bytes:
            raise AirflowException("Le fichier lu depuis HDFS est vide.")
        ...
        try:
            ...
            with zipfile.ZipFile(zip_buffer, "r") as zf:
                ...
                logger.info("Fichier texte trouvé dans le ZIP : %s", txt_name)

                with zf.open(txt_name) as txt_file:
                    txt_bytes = txt_file.read()
        except zipfile.BadZipFile as e:
            raise AirflowException(f"Le fichier téléchargé n'est pas une archive ZIP valide : {e}")
        ...
        cp_paris = {f"750{i:02d}" for i in range(1, 21)}
        cp_paris.add("75116")
        ...
        for chunk in chunks:
            ...
            manquantes = [c for c in colonnes_obligatoires if c not in chunk.columns]
            if manquantes:
                raise AirflowException(f"Colonnes manquantes dans le fichier DVF: {manquantes}")
            ...
            chunk["code_postal"] = chunk["code_postal"].astype(str).str.strip().str.replace(".0", "", regex=False)

            chunk_filtered = chunk[
                (chunk["type_local"] == "Appartement")
                & (chunk["nature_mutation"] == "Vente")
                & (chunk["code_postal"].isin(cp_paris))
                & (chunk["surface_reelle_bati"].between(9, 500, inclusive="both"))
            ...
            if sum(len(c) for c in filtered_chunks) >= 5000:
                break

        if not filtered_chunks:
            logger.info("Nombre de lignes avant filtrage : %s", total_rows)
            logger.info("Nombre de lignes après filtrage : 0")
            raise AirflowException(
                "Aucune ligne restante après filtrage. "
                "Le fichier lu ne contient pas encore assez de lignes Paris dans les chunks parcourus."
            )
        ...
        hook.run(
            """
            DELETE FROM dvf_raw
            WHERE annee_mutation = %s AND mois_mutation = %s
            """,
            parameters=(annee_courante, mois_courant),
        )
        ...
        rows_raw = []
        for _, row in df[raw_fields].iterrows():
            rows_raw.append(
                (
    ...
    def generer_rapport(nb_inseres: int) -> str:
        ...
        lignes = []
        lignes.append(f"Rapport DVF Paris - {mois:02d}/{annee}")
        lignes.append("")
        lignes.append("Arrondissement | Median (EUR/m2) | Moyen (EUR/m2) | Transactions | Surface moy.")
        lignes.append("-------------- | --------------- | -------------- | ------------ | ------------")

        for arrdt, mediane, moyenne, nb_tx, surface in records:
            lignes.append(
                f"{arrdt:>13} | "
        ...
        rapport = "\n".join(lignes)
        logger.info("Nombre de lignes insérées reçu : %s", nb_inseres)
        logger.info("\n%s", rapport)
        return rapport
